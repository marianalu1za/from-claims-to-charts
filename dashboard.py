...
from data_processing import load_and_prepare_data, READABLE_NAMES
...
MAX_DASH_ROWS = 50000
if len(df) > MAX_DASH_ROWS:
    df = df.sample(MAX_DASH_ROWS, random_state=42)
...
def make_patent_map(df_in: pd.DataFrame, map_scope: str, selected_state: str = None) -> go.Figure:
    """
    Create a choropleth map showing patent distribution.
    Supports world map or US state map with optional state filtering.
    
    Args:
        df_in: Filtered dataframe
        map_scope: 'world' or 'us'
        selected_state: Optional state code to filter (e.g., 'CA', 'NY')
    """
    ...
    if map_scope == 'world':
        ...
        country_counts = df_map['disambig_country'].value_counts().reset_index()
        country_counts.columns = ['country', 'patent_count']
        
        # Calculate better color range based on data distribution (similar to US map)
        min_count = country_counts['patent_count'].min()
        max_count = country_counts['patent_count'].max()
        # Use a more compressed range to make lower values more visible
        # This helps differentiate countries with lower patent counts
        if max_count > min_count:
            color_max = max(min_count + (max_count - min_count) * 0.3, max_count * 0.2)
        else:
            color_max = max_count
        
        # Use a darker color scale for better visibility
        # 'Viridis' or 'Plasma' are darker, or we can use a custom darker blue scale
        fig = px.choropleth(
            country_counts,
            locations='country',
            locationmode='country names',
            color='patent_count',
            color_continuous_scale='Viridis',  # Darker, more visible color scale
        ...
        # Customize hover template to show country name and patent count clearly
        fig.update_traces(
...
def make_histogram_kde(df_in: pd.DataFrame, column: str, bins: int = 30) -> go.Figure:
    ...
    fig.update_traces(marker_color='#00CC96', marker_line_color='#00A67E', marker_line_width=1, opacity=0.7)
    fig.update_layout(
        paper_bgcolor=STYLES['secondary_bg'],
        plot_bgcolor=STYLES['secondary_bg'],
        font=dict(family=STYLES['font_family'], size=12, color=STYLES['primary_text']),
...
def make_regression_plot(df_in: pd.DataFrame, x_col: str, y_col: str) -> go.Figure:
    ...
    fig = px.scatter(
        ...
        opacity=0.6,
        color_discrete_sequence=['#EF553B'],  # Red-orange
    )
    # Update trendline color
    fig.update_traces(marker_color='#EF553B', line_color='#FF6B35', line_width=3)
    fig.update_layout(
        paper_bgcolor=STYLES['secondary_bg'],
        plot_bgcolor=STYLES['secondary_bg'],
        font=dict(family=STYLES['font_family'], size=12, color=STYLES['primary_text']),
...
def make_contour_plot(df_in: pd.DataFrame, x_col: str, y_col: str) -> go.Figure:
    """Create contour plot (density plot)"""
    df_plot = df_in[[x_col, y_col]].dropna()
    if df_plot.empty:
...
def toggle_overview_state_filter(map_scope):
    """Show/hide state filter based on map scope"""
    if map_scope == 'us':
        return {'flex': '1', 'minWidth': '150px', 'display': 'block'}
    else:
        return {'flex': '1', 'minWidth': '150px', 'display': 'none'}
...
def update_overview_map_and_bar(map_scope, selected_state, scope_value, global_assignee_type, quarter_range):
    ...
    # Update bar plot - use map scope to determine what to show
    # If world scope, show all data; if US scope, show US data; if state selected, filter to that state
    if map_scope == 'world':
        bar_scope = 'ALL'
    elif map_scope == 'us':
        if selected_state and selected_state != 'ALL':
    ...
    bar_fig = make_overview_cpc_count(filtered, bar_scope, selected_state or None)
